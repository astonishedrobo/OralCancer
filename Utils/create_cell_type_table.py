...
def create_table(data, label=None):
    stats = pd.DataFrame()
    for patch, instances in data.items():
        patch_parts = patch.split("-")
        patient_id = f"{int(patch_parts[1])}-{int(patch_parts[2])}" 
        patch_name = f"{patch_parts[3]}{int(patch_parts[4])}{patch_parts[5][:-4]}"
        stat = {
    ...
    if parent_folder is not None:
        ...
        for folder in folders:
            if os.path.isdir(os.path.join(parent_folder, folder)):
                ...
                print(json_path)
                type_ = parent_folder.split("/")[-1].split("-")[-1]
                df = create_table(load_json(json_path), label=type_)
                if args.combine == True:
                    collated_df = pd.concat([collated_df, df], ignore_index=True)
                    collated_df.reset_index()
                
                else:
                    save_path = os.path.join(parent_folder, folder, "cell_stats.csv")
                    df.to_csv(save_path, index=False)

        if args.combine == True:
            save_path = os.path.join(parent_folder, "cell_stats_combined.csv")
