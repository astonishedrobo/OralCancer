...
from preprocessing.encoding.datasets.patched_wsi_inference import PatchedWSIInference, PatchInference
...
COLOR_MAP_JAMA = {
    "UNK": (255, 255, 255),
    "Neoplastic": (55,78,85),
    "Inflammatory": (223,143,68),
    "Connective": (0,161,213),
    "Dead": (178,71,69),
    "Epithelial": (121,175,151),
}
...
class CellSegmentationInference:
    ...
    def process_patches(
        ...
        dataset = PatchInference(patch_dir=patches_path, transform=self.inference_transforms)
        ...
        with torch.no_grad():
            for batch in tqdm.tqdm(dataloader):
                ...
                instance_map, instance_types, tokens = self.get_cell_predictions_with_tokens(
                    predictions
                )
                instance_map_gb.extend(instance_map)
                instance_types_gb.extend(instance_types)
                tokens_gb.extend(tokens)
                
        instance_segmentation_geogson = {}
        for i, (name, instance_types) in enumerate(zip(patch_names, instance_types_gb)):
            instance_segmentation_geogson[name] = {str(key): value for key, value in instance_types_gb[i].items()}

        instance_segmentation_geogson = self.fix2json(instance_segmentation_geogson)

        # # Save the Json
        os.makedirs(save_path, exist_ok=True)
        json_filename = os.path.join(save_path, "instance_segmentation.json")
        with open(json_filename, 'w') as json_file:
            json.dump(instance_segmentation_geogson, json_file, indent=2)

        print(f"The json file is saved to {json_filename}")

        # ## Save the tokens (feature vectors)
        # cell_embeddings = {}
        # for i, (name, tokens) in enumerate(zip(patch_names, tokens_gb)):
        #     cell_embeddings[name] = tokens

        ## Saves the cell embeddings directly
        cell_embeddings = {}
        for i, (name, tokens) in enumerate(zip(patch_names, tokens_gb)):
            for inst_seg in instance_segmentation_geogson[name].keys():
                bbox = instance_segmentation_geogson[name][inst_seg]['bbox']
                bb_index = np.array(bbox) / self.model.patch_size
                bb_index[0, :] = np.floor(bb_index[0, :])
                bb_index[1, :] = np.ceil(bb_index[1, :])
                bb_index = bb_index.astype(np.uint8)
                if name not in cell_embeddings.keys():
                    cell_embeddings[name] = {}
                cell_embeddings[name][inst_seg] = tokens[
                    :,
                    bb_index[0, 1] : bb_index[1, 1],
                    bb_index[0, 0] : bb_index[1, 0],
                ]

        hdf5_file_name = os.path.join(save_path, 'cell_embeddings.h5')

        # Compression options (adjust these as needed)
        compression_opts = {
            'compression': 'gzip',  
            'compression_opts': 9,
        }
        ...
        with h5py.File(hdf5_file_name, 'w') as hf:
            for name, inst_seg_dict in cell_embeddings.items():
                group = hf.create_group(name)
                for inst_seg, data in inst_seg_dict.items():
                    group.create_dataset(inst_seg, data=data, **compression_opts)

        file_size = os.path.getsize(hdf5_file_name)
        print(f"Cell embeddings saved to {hdf5_file_name}")
        print(f"File size: {file_size / (1024 * 1024):.2f} MB")
                
        # # Save the Segmentation Maps
        # os.makedirs(os.path.join(save_path, "pred_binary_masks"), exist_ok=True)
    ...
    def semantic_map_rgb(self, mask, mask_json):
        mask = mask.copy()
        m, n = mask.shape
        mask_rgb = np.zeros((m, n, 3))
        for key, value in mask_json.items():
            mask_rgb[mask == int(key)] = COLOR_MAP_JAMA[mask_json[key]["type"]]

        return mask_rgb
    
    def barplot(self, mask_json):
        class_dist = {}
        for key, value in mask_json.items():
            type_ = mask_json[key]["type"]

            if type_ in class_dist.keys():
                class_dist[type_] += 1
            else:
                class_dist[type_] = 1
        
        categories = list(class_dist.keys())
        values = list(class_dist.values())
        plt.figure(figsize=(5, 5))  # Adjust the figure size accordingly
        plt.barh(categories, values, color='blue')  # Use plt.barh for horizontal bars
        plt.tight_layout()

        return plt

    def fix2json(self, data: dict, type2name: bool = True) -> dict:
        """Convert numpy arrays to lists for json serialization

        Args:
            data (dict): Dictionary with numpy arrays
                * key (instance): str
                * value: dict
                    * bbox: np.ndarray
                    * centroid: np.ndarray
        ...
        Returns:
            dict: Dictionary with lists
            maps type (int) to names
        """
        for key, value in data.items():
            if isinstance(value, np.ndarray):
                data[key] = value.tolist()
            elif isinstance(value, dict):
                data[key] = self.fix2json(value)

            if key == "type":
                try:
                    data[key] = TYPE_NUCLEI_DICT[value]
                except:
                    continue
        return data
    ...
    def get_cell_predictions_with_tokens(
        ...
        return instace_map, instance_types, tokens
...
class InferenceWSIParser:
    ...
    def __init__(self) -> None:
        ...
        # Patches
        subparser_patches = subparsers.add_parser(
            "process_patches",
            description="Process a single patch file",
        )
        subparser_patches.add_argument(
            "--patch_path",
            type=str,
            help="Path to the patches folder",
        )
        subparser_patches.add_argument(
            "--save_path",
            type=str,
...
if __name__ == "__main__":
    ...
    if command.lower() == "process_patches":
        cell_segmentation.logger.info("Processing single patch file")
        patch_path = Path(configuration["patch_path"])
        batch_size = configuration["batch_size"]
        cell_segmentation.process_patches(patch_path, batch_size=batch_size, save_path=configuration["save_path"])
            
    elif command.lower() == "process_wsi":
